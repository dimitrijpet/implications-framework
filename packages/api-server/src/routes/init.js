...
function getTestPlannerTemplate() {
  const fileContent = `// Auto-generated by Implications Framework
...
buildPrerequisiteChain(ImplicationClass, currentStatus, targetStatus, visited = new Set(), isOriginalTarget = true) {
  ...
  if (meta.requires?.previousStatus) {
    ...
    if (meta.entity) {
      console.log(\`   ℹ️  Entity prerequisite: \${meta.entity}.status must be \${previousStatus.replace(/^[^_]+_/, '')}\`);
    } else {
...
static _getCurrentStatus(testData, targetImplication = null) {
  ...
  if (meta?.entity) {
    ...
    console.log(\`   ⚠️  No status found for entity: \${entity}\`);
  }
  ...
  const analysis = planner.analyze(ImplicationClass, testData);
    ...
    if (!analysis.ready && analysis.entityFields?.length > 0) {
      console.log(\`\\n⚠️  Missing entity requirements (handle in test setup):\`);
      analysis.entityFields.forEach(field => {
        const fieldName = typeof field === 'string' ? field : field.field;
        const required = typeof field === 'string' ? 'defined' : field.required;
        console.log(\`   - \${fieldName}: required=\${required}\`);
      });
      console.log(\`\\n💡 These should be set up in your test's beforeEach() or setup phase.\\n\`);
    }
  ...
  _buildPathObject(transitions, currentStatus, currentPlatform) {
    ...
    for (const transition of transitions) {
      ...
      const actionName = this._getActionNameForTransition(transition);
  ...
  static promptPathSelection(paths, currentStatus, targetStatus, timeoutSeconds = 10) {
    ...
    console.log(\`⏱️  Press Enter within \${timeoutSeconds} seconds (or it auto-selects Path 1)...\\n\`);
    
    try {
      const answer = readlineSync.question('Your choice: ', {
        limit: new RegExp(\`^[1-\${paths.length}]?$\`),
        limitMessage: \`Please enter 1-\${paths.length} or press Enter for default\`,
        defaultInput: '1'
      });
      ...
      console.log(\`\\n⏱️  Timeout! Auto-selecting Path 1 (recommended)\\n\`);
      return paths[0];
    }
  }
  ...
  static _displayPath(path, index, isRecommended) {
    ...
    path.steps.forEach((step, stepIndex) => {
      ...
      if (!step.complete && !step.isTarget && step.actionName) {
    ...
    const incompleteSteps = path.steps.filter(s => !s.complete && !s.isTarget).length;
  ...
  _loadDiscoveryCache() {
    try {
      ...
      console.log(\`   ⚠️  Error loading discovery cache: \${error.message}\`);
    }
    ...
    const planner = new TestPlanner({ verbose: true });
      ...
      if (!analysis.ready) {
        ...
        if (analysis.missingFields && analysis.missingFields.length > 0) {
          const hasEntityFields = analysis.missingFields.some(field => {
            const fieldName = typeof field === 'string' ? field : field.field;
            return fieldName && fieldName.includes('.');
          });
          
          if (hasEntityFields) {
            console.log(\`\\n⚡ Missing requirements will be auto-resolved during test:\`);
            analysis.missingFields.forEach(field => {
              const fieldName = typeof field === 'string' ? field : field.field;
              const required = typeof field === 'string' ? 'defined' : field.required;
              const actual = typeof field === 'string' ? 'missing' : field.actual;
              console.log(\`   - \${fieldName}: required=\${required}, actual=\${actual}\`);
            });
            console.log(\`\\n✅ Pre-flight check passed (auto-resolution enabled)\\n\`);
            return true;
          }
        }
        
        const allPaths = planner._findAllPaths(currentStatus, targetStatus, currentPlatform);
        ...
        if (allPaths.length === 1) {
          ...
          console.log(\`\\n⚡ Single path found to \${targetStatus}:\`);
          path.steps.forEach((step, i) => {
            const icon = step.complete ? '✅' : step.isTarget ? '🎯' : '📍';
            console.log(\`   \${i + 1}. \${icon} \${step.status}\`);
            if (!step.complete && !step.isTarget && step.actionName) {
              console.log(\`      └─ \${step.actionName} (\${step.platform})\`);
            }
          });
          
          console.log(\`\\n✅ Pre-flight check passed (auto-execution enabled)\\n\`);
          
          process.env.SELECTED_PATH_JSON = JSON.stringify(path);
          return true;
        }
        
        if (allPaths.length > 1) {
          console.log(\`\\n🔍 Found \${allPaths.length} possible paths to \${targetStatus}\\n\`);
          
          const selectedPath = TestPlanner.promptPathSelection(
            allPaths,
            currentStatus,
            targetStatus,
            10
          ...
          console.log(\`✅ Pre-flight check passed (path selected)\\n\`);
          
          process.env.SELECTED_PATH_JSON = JSON.stringify(selectedPath);
          return true;
        }
      }
