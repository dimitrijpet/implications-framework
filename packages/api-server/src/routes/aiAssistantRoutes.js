...
function generateScreenObjectCode(elements, screenName, options = {}) {
  ...
  // Generate locators - different syntax for web vs mobile
  const locators = elements.map(el => {
    const selector = el.selectors?.[0]?.value || el.label || el.name;
    
    if (isMobile) {
      // WebDriverIO / Appium style: $('~accessibilityId') or $('id=...')
      ...
      // If it looks like an accessibility ID (camelCase or dot.notation)
      else if (/^[a-z][a-zA-Z0-9.]*$/.test(selector)) {
        mobileSelector = `~${selector}`;
      }
      // Otherwise try accessibility ID
      else {
  ...
  // Build class - different constructor for web vs mobile
  if (isMobile) {
    // Mobile: No constructor needed if using global $
...
`;
    ...
    // Web: Playwright with page instance
    return `// ${className}.screen.js
// Generated by AI Assistant
