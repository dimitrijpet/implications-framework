...
router.post('/generate-screen-object', async (req, res) => {
  ...
  try {
    const code = generateScreenObjectCode(elements, screenName, { 
      format, 
      platform, 
      compoundMethods,  // ADD THIS
      ...style 
...
function generateScreenObjectCode(elements, screenName, options = {}) {
  ...
  const locators = elements.map(el => {
    const selector = el.selectors?.[0]?.value || el.label || el.name;
    
    if (isMobile) {
      let mobileSelector = selector;
      if (selector.startsWith('~') || selector.startsWith('id=') || selector.startsWith('//') || selector.startsWith('-ios')) {
        mobileSelector = selector;
      } else if (/^[a-z][a-zA-Z0-9.]*$/.test(selector)) {
        mobileSelector = `~${selector}`;
      } else {
        mobileSelector = `~${el.name}`;
      }
      
      return `  get ${el.name}() {
    return $('${mobileSelector.replace(/'/g, "\\'")}');
  }`;
    } else {
    ...
    return this.page.${selector};
      ...
      return `  get ${el.name}() {
    return this.page.locator('${selector.replace(/'/g, "\\'")}');
  }`;
    }
  }).join('\n\n');
  ...
  const actions = elements
    ...
    .map(el => {
      const methodName = el.name.charAt(0).toUpperCase() + el.name.slice(1);
      
      if (el.type === 'input') {
        if (isMobile) {
          return `  async fill${methodName}(value) {
    await this.${el.name}.setValue(value);
  }`;
        }
        return `  async fill${methodName}(value) {
  ...
  const assertions = includeAssertions ? elements.map(el => {
    ...
    return await this.${el.name}.isDisplayed();
  }`;
  }).join('\n\n') : '';

  // ═══════════════════════════════════════════════════════════════
  // GENERATE COMPOUND METHODS
  // ═══════════════════════════════════════════════════════════════
  let compoundMethodsCode = '';
  if (compoundMethods && compoundMethods.length > 0) {
    const methodsCode = compoundMethods.map(method => {
...
router.post('/merge-pom', async (req, res) => {
  ...
  // DEBUG - must come AFTER destructuring
  console.log('🔍 Merge POM request:', {
    filePath,
    diffNew: diff?.new?.length || 0,
    diffChanged: diff?.changed?.length || 0,
  ...
  try {
    ...
    console.log('🔍 Parsed POM:', {
      className: existingPOM.className,
      locatorsCount: existingPOM.locators?.length || 0,
      actionsCount: existingPOM.actions?.length || 0,
      compoundMethodsCount: existingPOM.compoundMethods?.length || 0,
      isMobile: existingPOM.isMobile
    });
    ...
    const newContent = await pomComparator.merge(existingPOM, diff, {
      ...options,
      compoundMethods
    });
    ...
    const addedCompoundMethods = compoundMethods?.length || 0;
